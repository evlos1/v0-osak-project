...
// import i18n from "@/i18n"; // <-- 직접 import 제거

export default function TopicSelectionPage() {
  const router = useRouter();
  // useTranslation 훅에서 i18n 인스턴스를 함께 가져옵니다.
  ...
  const detailCategories: { [key: string]: string[] } = {
    ...
    사진: ["풍경사진", "인물사진", "다큐멘터리사진", "예술사진", "상업사진"],
  ...
  const renderStepContent = () => {
    switch (step) {
        ...
        return (
          <>
            <h2 className="text-2xl font-bold mb-6">
              {/* `i18nInstance.language` 사용으로 변경 */}
              {i18nInstance.language === "ko"
                ? `${selectedCategory}의 세부 분야를 선택하세요`
                : t("select_subcategory_of", { category: t(selectedCategory) })}
            </h2>
      ...
      case 3:
        if (!selectedSubCategory || !detailCategories[selectedSubCategory]) return null;
        return (
          <>
            <h2 className="text-2xl font-bold mb-6">
              {/* `i18nInstance.language` 사용으로 변경 */}
              {i18nInstance.language === "ko"
                ...
                : t("select_detail_of", { category: t(selectedSubCategory) })}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
